...
const AddBookModal = ({ onClose, onAdd }) => {
  const [BOOK_NM_INFO, setBOOK_NM_INFO] = useState('');
  const [AUTHOR_NM_INFO, setAUTHOR_NM_INFO] = useState('');
  const [PUBLSHCMPY_NM, setPUBLSHCMPY_NM] = useState('');
  const [PUBLCATN_YY, setPUBLCATN_YY] = useState('');
  const [BOOK_IMAGE_URL, setBOOK_IMAGE_URL] = useState('');
